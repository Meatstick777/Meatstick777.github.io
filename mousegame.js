...
function draw() {
  if (gameState === "intro") {
    ...
    // Draw background per level with unique wall colors
    if (level === 1) drawBackground(color(0, 0, 255));      // blue lines
    if (level === 2) drawBackground(color(255, 0, 0));      // red lines
    if (level === 3) drawBackground(color(0, 255, 255));    // cyan lines
    if (level === 4) drawBackground(color(255, 255, 0));    // yellow lines
...
function drawIntroScreen() {
  ...
  text("2. Avoid red ghosts—they deduct points.", width / 2, height / 2 + 20);
...
function handleGhosts() {
  ...
  // Continuously prevent ghosts from overlapping
  separateGhosts();
}

// ----------------------------
// SEPARATE GHOSTS (to avoid stacking)
